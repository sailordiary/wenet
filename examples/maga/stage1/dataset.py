...
from examples.maga.stage1.client import FileHandler
from wenet.dataset import processor
from wenet.dataset.datapipes import WenetRawDatasetSource
from wenet.ssl.init_dataset import padding


def get_wav_from_ceph(sample, client=None):
    if client is None:
        return sample
    aws_key = sample['wav']
    wav_bytes = client.get_file_bytes(aws_key)
    sample['wav'] = wav_bytes
    return sample


def CephDataset(data_type, data_list_file, conf=None, partition=True):
    ...
    client = FileHandler(os.getenv('AWS_ACCESS_KEY', None),
                         os.getenv('AWS_SECRET_KEY', None),
                         os.getenv('AWS_ENDPOINT', None))
    dataset = dataset.map_ignore_error(
        partial(get_wav_from_ceph, client=client))
...
def init_dataset(data_type, data_list_file, conf=None, partition=True):
    return CephDataset(data_type, data_list_file, conf, partition)
